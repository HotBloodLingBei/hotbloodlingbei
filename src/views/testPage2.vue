...
<template>
    <div style="padding: 5%">
        <div class="questionContainer">
            ...
            <div style="font-family: 站酷仓耳渔阳体,serif;font-size: 1.6em;  height:  160px;  display: flex; justify-content: center;align-items: center;">
            ...
            <div class=options
                ...
                <button
                        :style="{width:'100px' , height: '45px',background: store.Scores[currentQuestionIndex].value===questionsAccurateVersion.optionScore[1].score ? '#bebebe':'#ffffff'}"
                        @click="fetchChoice(1)" class="button">{{ questionsAccurateVersion.optionScore[1].text }}
                </button>
                <button
                        :style="{width:'100px' , height: '45px',background: store.Scores[currentQuestionIndex].value===questionsAccurateVersion.optionScore[2].score ? '#bebebe':'#ffffff'}"
                        @click="fetchChoice(2)" class="button">{{ questionsAccurateVersion.optionScore[2].text }}
                </button>
                <button
                        :style="{width:'100px' , height: '45px',background: store.Scores[currentQuestionIndex].value===questionsAccurateVersion.optionScore[3].score&& store.Scores[currentQuestionIndex].valid===1 ? '#bebebe':'#ffffff'}"
                        @click="fetchChoice(3)" class="button">{{ questionsAccurateVersion.optionScore[3].text }}
                </button>
                <button
                        :style="{width:'100px' , height: '45px',background: store.Scores[currentQuestionIndex].value===questionsAccurateVersion.optionScore[4].score ? '#bebebe':'#ffffff'}"
                        @click="fetchChoice(4)" class="button">{{ questionsAccurateVersion.optionScore[4].text }}
                </button>
                <button
                        :style="{width:'100px' , height: '45px',background: store.Scores[currentQuestionIndex].value===questionsAccurateVersion.optionScore[5].score ? '#bebebe':'#ffffff'}"
                        @click="fetchChoice(5)" class="button">{{ questionsAccurateVersion.optionScore[5].text }}
                </button>
                <button
                        :style="{width:'100px' , height: '45px',background: store.Scores[currentQuestionIndex].value===questionsAccurateVersion.optionScore[6].score ? '#bebebe':'#ffffff'}"
                        @click="fetchChoice(6)" class="button">{{ questionsAccurateVersion.optionScore[6].text }}
                </button>
            </div>
        </div>
        <!--按钮容器-->
        <div style="font-family: LongZhuTi-Regular,serif; align-items: center;justify-content: space-between;display: flex;">
            ...
            <!--保存进度按钮-->
            <div style="font-family: LongZhuTi-Regular,serif;margin: 10px;display: flex;height: 60px; width: 100px;  align-items: center;">
          ...
          <span class="IconContainer">
            ...
            <div style="height: 60px; width: 100px; display: flex;align-items: center;">
                <button class="shadow__btn" @click="handleSubmit"
                        v-if="currentQuestionIndex === questionsAccurateVersion.questionList.length-1">提交
                </button>
            </div>
            <!--按钮容器结束-->
        </div>
        <div>{{ currentQuestionIndex + 1 }} / {{ questionsAccurateVersion.questionList.length }}</div>
        <!--进度条的容器-->
        <div style="margin:10px;background-color: white;display: flex;align-items: flex-start;">
            <button class="left-aligned-button"
                    :style="{height:'100%', width:((currentQuestionIndex+1)*100/questionsAccurateVersion.questionList.length) + '%' }"/>
        </div>
